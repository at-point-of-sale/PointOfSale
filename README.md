...
The website runs on http://localhost:8787 and the apps reached over an origin come from their live hostnames. `npm run dev:apps` is for apps reached over a binding: it builds the three apps from the sibling checkouts (`../ReceiptPrinterPlayground` and so on) and runs each as a wrangler dev process next to the website, so the bindings connect locally.
